...
SAML.prototype.checkSAMLStatus = function (xmlDomDoc) {
    ...
    console.warn('[SAML] Failed to find StatusCode in XML: ' + err);
  }

	if(statusCodeValueNode != null || undefined){
		status.StatusCodeValue  = statusCodeValueNode.getAttribute('Value');
	}else{
    console.warn('[SAML] statusCodeValueNode is not defined.');
  }

	try{
    var statusMessageNode = xmlCrypto.xpath(xmlDomDoc, "//*[local-name(.)='StatusMessage']")[0];
  }catch(err){
    console.warn('[SAML] Failed to find StatusMessage in XML: ' + err);
  }

	if(statusMessageNode != null || undefined){
		status.StatusMessage  = statusMessageNode.childNodes[0].nodeValue;
	}else{
    console.warn('[SAML] statusMessageNode is not defined.')
  }

	try{
    var statusDetailNode = xmlCrypto.xpath(xmlDomDoc, "//*[local-name(.)='StatusDetail']/*[local-name(.)='Cause']")[0];
  }catch(err){
    console.warn('[SAML] Failed to find StatusDetail in XML: ' + err);
  }

	if(statusDetailNode != null || undefined){
		status.StatusDetail  = statusDetailNode.childNodes[0].nodeValue;
	}else{
    console.warn('[SAML] statusDetailNode is not defined.');
  }
...
SAML.prototype.decryptSAMLAssertion = function (xmlDomDoc, privateCert) {
    ...
    console.warn('[SAML] Failed to find EncryptedData in XML: ' + err);
  }
...
SAML.prototype.validateSignature = function (xml, cert) {
  ...
  try{
    ...
    console.warn('[SAML] Failed to find Signature in XML: ' + err);
  }
  
  try{
    var sig = new xmlCrypto.SignedXml();
  }catch(err){
    console.warn('[SAML] Failed to create Signed XML: ' + err);
  }
...
SAML.prototype.validateResponse = function (samlResponse, callback) {
  ...
  var samlAssertion = null;
    ...
    console.log('[SAML] Invalid Signature!');
	}
	...
	if (self.options.encryptedSAML &&  self.options.privateCert)
	{
        ...
        console.log('[SAML] Decryption Failed!');
		}
	...
	else //Retrieve SAML Assertion
	{
      ...
      console.warn('[SAML] Failed to find Assertion in XML: ' + err);
    }
